...
echo -e "${YELLOW}=============================================${RESET}"
echo -e "${YELLOW}           [0] Check R and R libs            ${RESET}"
echo -e "${YELLOW}=============================================${RESET}"
AVAIL_R_PATH=false
if command -v Rscript > /dev/null 2>&1; then
  R_LIB_PATHS=$(Rscript -e "cat(.libPaths(), sep = ':')")
  echo -e "${BLUE}[0:1] R library paths: ${GREEN}$R_LIB_PATHS${RESET}"

  IFS=':' read -r -a paths <<< "$R_LIB_PATHS"
  for path in "${paths[@]}"; do
    if [ -w "$path" ]; then
      echo -e "${BLUE}[0:2] ${GREEN}$path${BLUE} is writable${RESET}"
      AVAIL_R_PATH=true
      break
    else
      echo -e "${BLUE}[0:3] ${GREEN}$path${BLUE} is not writable${RESET}"
    fi
  done
else
  echo -e "${RED}[0:4] No R module found!${RESET}"
  exit 1
fi
echo $AVAIL_R_PATH
if [[ "$AVAIL_R_PATH" == false ]]; then
  echo -e "${RED}[0:4] No writable R paths!${RESET}"
  exit 2
fi
R_VERSION=$(Rscript -e 'cat(paste0(sessionInfo()$R.version$major, ".", sessionInfo()$R.version$minor))')
echo -e "${BLUE}[0:END] R version found: $R_VERSION${RESET}\n\n"


echo -e "${YELLOW}==============================================${RESET}"
echo -e "${YELLOW}[0-1] Set passwordless connection to localhost${RESET}"
echo -e "${YELLOW}==============================================${RESET}"
if ssh -o BatchMode=yes localhost true; then
  echo -e "${BLUE}SSH connection successful without password.${RESET}\n\n"
else
  echo -e "${RED}SSH connection requires a password! Please make sure that you can passwordless connection to localhost!${RESET}"
  exit 3
fi
...
echo -e "${YELLOW}=============================================${RESET}"
echo -e "${YELLOW}                  [2] JAVA 8                 ${RESET}"
echo -e "${YELLOW}=============================================${RESET}"
cd $DEPEN_DIR
if [ ! -d "java" ] && [ ! -d "java/jdk1.8.0_201" ]; then
  echo -e "${BLUE}[2:1] There is no required java!${RESET}"
  rm -rf java
  mkdir java
  cd java
  echo -e "${BLUE}[2:2] Installing java 8${RESET}"
  wget https://compss.bsc.es/~fconejer/jdk-8u201-linux-x64.tar.gz
  tar -zxvf jdk-8u201-linux-x64.tar.gz
  rm jdk-8u201-linux-x64.tar.gz
else
  echo -e "${BLUE}[2:3] The required version of java already exists!${RESET}"
  cd java
fi
export JAVA_HOME=$PWD/jdk1.8.0_201
echo 'export JAVA_HOME='$PWD'/jdk1.8.0_201' >> $SETUP_DIR/config.sh
echo -e "${BLUE}[2:4] JAVA_HOME=$PWD/jdk1.8.0_201${RESET}"
export PATH=$PATH:$JAVA_HOME/bin
echo "export PATH=\$PATH:$JAVA_HOME/bin" >> $SETUP_DIR/config.sh
echo -e "${BLUE}[2:5] PATH=$PATH:$JAVA_HOME/bin${RESET}"
echo -e "${BLUE}[2:END] Finished installing JAVA\n\n${RESET}"
...
if [[ "$NEED_MAVEN" == true ]]; then
  MAVEN_VER="3.9.9"
  echo -e "${YELLOW}=============================================${RESET}"
  echo -e "${YELLOW}              [3-1] Maven-$MAVEN_VER         ${RESET}"
  echo -e "${YELLOW}=============================================${RESET}"
  cd $DEPEN_DIR
  if [ -d "maven" ] && [ -d "maven/apache-maven-$MAVEN_VER" ]; then
    echo -e "${BLUE}[3-1:1] Maven-$MAVEN_VER is already installed!${RESET}"
    cd maven
  else
    rm -rf maven
    mkdir maven
    cd maven
    echo -e "${BLUE}[3-1:2] Installing Maven-$MAVEN_VER at $PWD${RESET}"
  ...
  echo -e "${BLUE}[3-1:3] M2_HOME=$PWD${RESET}"
  export PATH="$M2_HOME/bin:$PATH"
  echo "export PATH=\$PATH:$M2_HOME/bin" >> $SETUP_DIR/config.sh
  echo -e "${BLUE}[3-1:4] PATH=$PATH${RESET}"
  echo -e "${BLUE}[3-1:END] Finished installing Maven${RESET}\n\n"
fi
...
if [[ "$NEED_GRADLE" == true ]]; then
  GRADLE_VER="5.4.1"
  echo -e "${YELLOW}=============================================${RESET}"
  echo -e "${YELLOW}              [3-2] Gradle-$GRADLE_VER       ${RESET}"
  echo -e "${YELLOW}=============================================${RESET}"
  cd $DEPEN_DIR
  if [ -d "gradle" ] && [ -d "gradle/gradle-$GRADLE_VER" ]; then
    echo -e "${BLUE}[3-2:1] Gradle-$GRADLE_VER is already installed!${RESET}"
    cd gradle
  else
    rm -rf gradle
    mkdir gradle
    cd gradle
    echo -e "${BLUE}[3-2:2] Installing Gradle-$GRADLE_VER at $PWD${RESET}"
  ...
  echo -e "${BLUE}[3-2:3] PATH=$PATH${RESET}"
  echo -e "${BLUE}[3-2:END] Finished installing Gradle${RESET}\n\n"
fi


BUILD_FROM_SOURCE=false
echo -e "${YELLOW}=============================================${RESET}"
echo -e "${YELLOW}                  [4] COMPSs                 ${RESET}"
echo -e "${YELLOW}=============================================${RESET}"
# Create the installation folder
cd $DEPEN_DIR
COMPSs_EXISTS=false
if [ -d "COMPSs_installation" ]; then
  echo -e "${BLUE}[4:1] The directory 'COMPSs_installation' already exists!${RESET}"
  if ask_yes_no "[4:2] Do you want to reinstall COMPSs?"; then
  ...
  mkdir COMPSs_installation
  echo -e "${BLUE}[4:3] Created folder 'COMPSs_installation'${RESET}"
fi
cd COMPSs_installation
COMPSs_INSTALL_DIR=$PWD
echo -e "${BLUE}[4:4] COMPSs_INSTALL_DIR=$PWD${RESET}"

cd $DEPEN_DIR
if [[ "$COMPSs_EXISTS" == false ]]; then
  if ask_yes_no "[4:5] Do you want to build from binary?"; then
    if [ -d "compss" ]; then
      echo -e "${BLUE}[4:6] The directory 'compss' already exists!${RESET}"
    else
      echo -e "${BLUE}[4:7] Cloning 'compss'${RESET}"
      git clone git@github.com:bsc-wdc/compss.git
    fi
    cd compss
    git checkout r_binding
    echo -e "${BLUE}[4:8] Switched to branch 'r_binding'${RESET}"

    # Update the submodules
    ./submodules_get.sh
    echo -e "${BLUE}[4:9] Updated the submodules${RESET}"
...
echo -e "${YELLOW}=============================================${RESET}"
echo -e "${YELLOW}                     [5] R                   ${RESET}"
echo -e "${YELLOW}=============================================${RESET}"
echo -e "${BLUE}[5:1] R version specified: $R_VERSION${RESET}"
...
echo -e "${BLUE}[5:3] Finished loading R${RESET}"
echo -e "${BLUE}[5:4] Installing R dependencies Rcpp, RMVL, pryr and proxy${RESET}"
Rscript -e "options(repos = c(CRAN = 'https://cloud.r-project.org')); list.of.packages <- c('Rcpp', 'RMVL', 'pryr', 'proxy'); new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,'Package'])]; if(length(new.packages)) install.packages(new.packages)"
echo -e "${BLUE}[5:END] R loaded with dependencies!${RESET}\n\n"
