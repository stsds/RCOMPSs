...
echo -e "${YELLOW}=============================================${RESET}"
echo -e "${YELLOW}             [0] Check the flags             ${RESET}"
echo -e "${YELLOW}=============================================${RESET}"
  ...
  case "$1" in
    --R-version)
      if [[ -n "$2" && "$2" != -* ]]; then
        ...
        echo -e "${BLUE}[0:1]${RESET} Error: --R-version requires a non-empty argument."
        usage
      fi
      ;;
    *)
      echo -e "${BLUE}[0:2]${RESET} Error: Unknown option $1"
      usage
      ;;
  esac
done
echo -e "${BLUE}[0:END] R module found: $R_VERSION${RESET}\n\n"
...
echo -e "${YELLOW}=============================================${RESET}"
# Check if R_VERSION is set
if [[ -z "$R_VERSION" ]]; then
  echo -e "${RED}[5:1] Error: --R-version is required.${RESET}"
  usage
fi
echo -e "${BLUE}[5:2] R version specified: $R_VERSION${RESET}"
module load gcc
module load mkl
...
echo "module load $R_VERSION" >> $SETUP_DIR/config.sh
